...
func (prwe *prwExporter) export(ctx context.Context, requests map[string][]*prompb.WriteRequest) error {
	chanLength := 0
		...
		go func() {
			...
			for {
				...
				case request, ok := <-input:
					...
					if errExecute := prwe.execute(ctx, request.string, request.WriteRequest); errExecute != nil {
						mu.Lock()
