...
func (prwe *prwExporter) handleExport(ctx context.Context, tsMap map[string]*prompb.TimeSeries) error {
	...
	if !prwe.walEnabled() {
		...
		for tenant, tenantRequests := range requests {
			// Perform a direct export otherwise.
			err := prwe.export(ctx, tenantRequests, tenant)
			if err != nil {
				errs = multierr.Append(errs, err)
			}
		}
...
func (prwe *prwExporter) export(ctx context.Context, requests []*prompb.WriteRequest, tenant string) error {
...
func (prwe *prwExporter) execute(ctx context.Context, tenant string, writeReq *prompb.WriteRequest) error {
	...
	if prwe.multiTenancy.Enabled && prwe.multiTenancy.QueryParam != "" {
		queryValues := endpointQueryUrl.Query()
		queryValues.Set(prwe.multiTenancy.QueryParam, tenant)
		endpointQueryUrl.RawQuery = queryValues.Encode()
	}
