...
func addLabelOp(metric pmetric.Metric, op internalOperation, f internalFilter) {
	rangeDataPointAttributes(metric, func(attrs pcommon.Map) bool {
		if _, found := attrs.Get(op.configOperation.NewLabel); !found {
			if newValue := f.expand(op.configOperation.NewValue, metric.Name()); newValue != "" {
				attrs.PutStr(op.configOperation.NewLabel, newValue)
			} else {
